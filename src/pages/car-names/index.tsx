...
import { getCarNames, deleteCarNameById } from 'apiSdk/car-names';
import { CarNameInterface } from 'interfaces/car-name';

type ColumnType = ColumnDef<CarNameInterface, unknown>;

interface CarNameListPageProps {
...
export function CarNameListPage(props: CarNameListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CarNameInterface>>(
    () => `/car-names?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CarNameInterface) => {
    if (hasAccess('car_name', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/car-names/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/car-names/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('car_name', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/car-names/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('car_name', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();
    ...
    <Flex direction="column" gap={{ md: 6, base: 7 }} shadow="none">
      <Flex justifyContent={{ md: 'space-between' }} direction={{ base: 'column', md: 'row' }} gap={{ base: '28px' }}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="car_name" />
        </Flex>

        {hasAccess('car_name', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/car-names/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
