...
import { getCarNames } from 'apiSdk/car-names';
...
import { CarNameListPage as ShowCaseList } from 'pages/car-names';

function DashboardPage() {
  const { session } = useSession();

  const carNameStat = useSWR<PaginatedInterface<CarNameInterface>>(
    () => `/car-names`,
    () => getCarNames({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Car Names',
      result: carNameStat?.data?.totalCount,
      isLoading: carNameStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`car-names`} passHref legacyBehavior>
                  <Button
